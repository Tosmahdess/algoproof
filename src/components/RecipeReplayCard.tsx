...
// hard-gate outcome of any kind — the corrected engine will judge these
// recipes again. The PF is the one of the selection universe (30 assets), so
// the card says it is not the bot's own basket.
...
export default function RecipeReplayCard({ view, tradedAssets }: {
  ...
  return (
    <aside data-testid="recipe-replay"
      ...
      <p>
        {ORIGIN} (trois défauts, trouvés début septembre). Le {day(view.replayedOn)}, j&apos;ai
      ...
      <p className="text-muted">
        ...
        hasard, et le moteur corrigé va rejuger cette stratégie en entier ; ce bloc sera
        remplacé par ce qu&apos;il trouve.
      </p>
      <p className="text-xs text-muted font-mono">empreinte du moteur : {view.engineFingerprint}</p>
    </aside>
  )
}
