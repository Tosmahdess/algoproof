...
const STORAGE_KEY = 'wealth_explainer_open'
...
export function ExplainerSignal() {
  ...
  return (
    <div className="mb-4 rounded-xl border border-zinc-800 bg-zinc-950">
      <button
        onClick={toggle}
        className="w-full flex items-center justify-between px-4 py-3 text-left hover:bg-zinc-900/50 rounded-xl transition-colors"
      >
        <span className="text-sm font-medium text-zinc-300">
          📖 Comment les signaux sont calculés
        </span>
        <span className="text-zinc-500 text-xs">{open ? '▲ Fermer' : '▼ Voir'}</span>
      </button>
