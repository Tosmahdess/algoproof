  ...
  return (
    <section className="rounded-xl border border-foreground/10 bg-foreground/[0.03] p-5">
      ...
      <div className="mt-4 grid grid-cols-2 gap-3 text-sm sm:grid-cols-4">
        <div><div className="text-foreground/50">Signal</div><div className="font-semibold">{signal}</div></div>
        <div><div className="text-foreground/50">Recul 180j</div><div className="font-semibold">{dd}</div></div>
        <div><div className="text-foreground/50">TP1</div><div className="font-semibold text-[#3fb950]">+{market?.tp1_pct ?? '—'}%</div></div>
        <div><div className="text-foreground/50">TP2</div><div className="font-semibold text-[#3fb950]">+{market?.tp2_pct ?? '—'}%</div></div>
      </div>
      <p className="mt-4 border-l-2 border-accent pl-3 text-foreground/90 italic">{fiche.verdict_reason}</p>
    </section>
  )
}
