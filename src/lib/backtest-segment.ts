...
// - the paper curve continues from the backtest's last capital by ADDING its euros, so the
//   amounts on the curve are the amounts in the paper trade list.
//
...
export function joinSegments(
  ...
  const shift = last.capital - startCapital
  ...
  for (let d = nextDay(last.date); lastPaperDate && d <= lastPaperDate; d = nextDay(d)) {
    ...
    rows.push({ date: d, backtest: null, paper: round2(capital + shift) })
  }
  return rows
}
