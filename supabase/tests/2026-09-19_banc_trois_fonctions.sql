-- BANC D'ESSAI -- les trois fonctions dos à dos, même transaction, mêmes conditions.
-- Transaction annulée : rien n'est conservé.
--
-- Pourquoi dos à dos : le 1,49 s de la branche payante date d'hier soir, le 11,8 s du
-- catalogue d'aujourd'hui, et la campagne de publication réécrit la table entre les deux.
-- Comparer ces chiffres, c'est comparer deux états de charge autant que deux fonctions.

begin;
set local statement_timeout = '300s';

-- identité d'abonné, locale à la transaction : la branche payante doit être exercée
...
create or replace function public.survivor_family_catalog_pl(
  p_dataset text default null, p_strategy text default null
...
create or replace function pg_temp.banc() returns table(etape text, octets int, secondes numeric)
language plpgsql as $B$
declare t timestamptz; n int;
begin
  t := clock_timestamp();
  n := pg_catalog.length(public.survivor_family_all(null, 'EMAcross')::text);
  etape := '1. branche PAYANTE (plpgsql, avec variantes)'; octets := n;
  secondes := round(extract(epoch from clock_timestamp() - t)::numeric, 2); return next;

  t := clock_timestamp();
  n := pg_catalog.length(public.survivor_family_catalog_pl(null, 'EMAcross')::text);
  etape := '2. jumeau TEASER en plpgsql'; octets := n;
  secondes := round(extract(epoch from clock_timestamp() - t)::numeric, 2); return next;

  t := clock_timestamp();
  n := pg_catalog.length(public.survivor_family_catalog(null, 'EMAcross')::text);
  etape := '3. catalogue actuel en sql (temoin)'; octets := n;
  secondes := round(extract(epoch from clock_timestamp() - t)::numeric, 2); return next;

  t := clock_timestamp();
  n := pg_catalog.length(public.survivor_family_catalog_pl(null, 'EMAcross')::text);
  etape := '4. jumeau plpgsql, 2e appel (cache de plans chaud)'; octets := n;
  secondes := round(extract(epoch from clock_timestamp() - t)::numeric, 2); return next;

  t := clock_timestamp();
  n := pg_catalog.length(public.survivor_family_catalog_pl(null, 'HMAcross')::text);
  etape := '5. jumeau plpgsql, la plus GROSSE strategie'; octets := n;
  secondes := round(extract(epoch from clock_timestamp() - t)::numeric, 2); return next;
end
$B$;

select * from pg_temp.banc();
